...
main() {
    ...
    while read -r pddl_instance; do
        ...
        #for hor in 600 ; do
            ...
            clingcon_output="${pddl_instance%.pddl}_pddl-bnd_clingcon_$hor.txt"
            echo $asp_facts > $clingcon_output
            echo $asp_facts | clingcon $asp_instance ./../../model/instance_random.lp  ./../../model/pddl_clingcon_bnd.lp - --const horizon=$hor --const bound=0 --config=crafty --time-limit=600 --q=1  >> $clingcon_output 2>/dev/null
