...
function select_instances
{
    ...
    prefix="./Instancesv2_round/sippv2/"
    len=${#prefix}

    for sufx in "random/" "sipp/" "sippv2/"; do
        tail -n +2 $task1 | while IFS=',' read -r HORIZON PROBLEM MIN; do
            if [[ "$PROBLEM" == *"_round"*"$scenario"*  && "$HORIZON" == "$horizon" ]]; then
               problem="./Instancesv2_round/${sufx}${PROBLEM:len}" #;echo $problem
               #echo "${dir}${problem}_${label}_$HORIZON.txt" 
...
function print_pddline
{
    local results=$1
    printf "pddl,${horizon},${asp_out}${results}"$'\n' >> $csv
    
}

function find_pddl_solution
{
    local -i len start end 
    local -n line_=$1 
    local pddl_value
    ...
    local -i tot_value
    for link in "${path[@]}"; do 
        if [[ "$line_" == *"pddl_solution(link(${link}),"* ]]; then    
          pddl_sol="pddl_solution(link(${link}),"
          calculate_position pddl_sol line_ len start end
          calc_atom_value pddl_value end line_ 
          #echo "${line_:$start:$len}${pddl_value}"
          tot_value+=$pddl_value 
          linelk="${linelk}${pddl_value[@]},"
        else
          linelk="${linelk},"
        fi
    done
    if [[ "${linelk}" != ",,,,," ]]; then
        #echo "${linelk}${tot_value}"
        print_pddline ",${linelk}${tot_value}"
    fi
...
function read_pddl
{
    ...
    tail -n +1 "${_asp_out}" | while read -r line; do 
          find_pddl_solution line      
    done
}

function calculate_tot_pddl
{
    for asp_out in "${asp_outputs[@]}"; do
        read_pddl asp_out
    done
