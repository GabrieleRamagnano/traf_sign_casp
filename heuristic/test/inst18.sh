...
path=("n43121,d,n43221"
      ...
      "n44151,h,n44211"
      "n44211,d,n44221")
csv="${dir}${home}/${label}_${horizon}.csv"
test="fixed-test-"
tnum_s=("5" "10" "14" "15" "17" "18")
horizon_s=("600" "660" "720" "780" "840" "900")

function set_csv { 
    bash "${utility}" search "${dir}","${home}" || mkdir "${dir}${home}"
    bash "${utility}" search "${dir}${home}","${label}_${horizon}.csv" && 
        ...
        for p in "p01" "p02" "p03" "p04" "p05" "p06" "p07"; do
            for hrz in "${horizon_s[@]}"; do
                if [[ $horizon == $hrz ]]; then
                    ...
                    is_there "${dir}${problem}_${label}_$hrz.txt"  #;echo "${dir}${problem}_${label}_$HORIZON.txt"  
                fi
            done                           
        done  
    done
